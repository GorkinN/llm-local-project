...
const DEFAULT_SYSTEM =
  "You are an image generation script generator. Your task is to write Python scripts that generate images" +
  " using diffusion models (Flux, Stable Diffusion XL, etc.) via the diffusers library from Hugging Face." +
  " Output only raw Python code without explanations, markdown wrappers, or introductions." +
  " Include proper error handling, GPU detection, and batch generation support.";
...
function writeScriptToFile(outputPath, script) {
...
async function main() {
  ...
  try {
    ...
    const rawScript = response.response || "";
    const script = stripMarkdownFences(rawScript);
    const tokensGenerated = response.eval_count || 0;

    if (outputPath) {
      const savedPath = writeScriptToFile(outputPath, script);
      process.stdout.write(
        JSON.stringify({
          success: true,
          model,
